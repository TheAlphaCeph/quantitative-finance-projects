...
class GBFModel(BaseModel):
    """GPU-accelerated Gradient Boosting with Optuna optimization"""
    ...
    def fit(self, train_data):
        # 确保所有数据转换为 CPU 的 NumPy 数组
        train_data = train_data.to_pandas() if isinstance(train_data, cudf.DataFrame) else train_data
    ...
    def _objective(self, trial, X, y):
        ...
        for train_idx, val_idx in tscv.split(X):
            ...
            scores.append(-model.score(X_val, y_val))  # 使用MAE需要负号
            
        return np.mean(scores)

    def predict(self, test_data):
        # 确保所有数据转换为 CPU 的 NumPy 数组
        test_data = test_data.to_pandas() if isinstance(test_data, cudf.DataFrame) else test_data
