...
def compute_resid_info(subdf: pl.DataFrame, x_cols: List[str], y_col: str) -> pl.DataFrame:
    """Computes residuals from an OLS regression."""
    X = subdf.select(x_cols).to_pandas()
    y = subdf.select(y_col).to_pandas().squeeze()
    X = sm.add_constant(X)
    model = sm.OLS(y, X).fit()
    residuals = model.resid.values
    return subdf.with_columns(pl.Series(f"{y_col}_resid", residuals))


def create_quintile_portfolios(df: pl.DataFrame, resid_col: str = "ML_resid") -> pl.DataFrame:
    """Assigns stocks to quintiles based on residuals within 3-digit SIC groups."""
    df = df.with_columns(
        pl.col("sic")
        .cast(pl.Utf8)
        .str.slice(0, 3)
    ...
    bounds = (
        ...
        .agg([
            pl.col(resid_col).quantile(0.2).alias("q20"),
            pl.col(resid_col).quantile(0.4).alias("q40"),
            pl.col(resid_col).quantile(0.6).alias("q60"),
            pl.col(resid_col).quantile(0.8).alias("q80")
        ])
    )
    ...
    df = df.with_columns(
        pl.when(pl.col(resid_col) <= pl.col("q20"))
        .then(1)
        .when(pl.col(resid_col) <= pl.col("q40"))
        .then(2)
        .when(pl.col(resid_col) <= pl.col("q60"))
        .then(3)
        .when(pl.col(resid_col) <= pl.col("q80"))
        .then(4)
        .otherwise(5)
        .alias("quintile")
    )
...
def main():
    ...
    cols_to_keep = ['gvkey', 'year', 'E', 'mkt_cap', 'E_future', 'E_pred_ols', 'E_pred_ann_bagging', 'E_pred_ar', 'E_pred_catboost', 'E_pred_gbf',
                    'E_pred_rf', 'E_pred_hvz', 'E_pred_lasso', 'E_pred_lightgbm', 'E_pred_ri', 'E_pred_ridge', 'E_pred_rw', 'E_pred_xgb', 'COMP_EXT', 'COMP_LR',
                    'COMP_NL', 'COMP_ML', 'COMP_MED_EXT', 'COMP_MED_LR', 'COMP_MED_NL', 'COMP_MED_ML', 'date', 'sic', 'retadj']
    df_merged = df_merged.select(cols_to_keep)


    # --- Configuration for residual computation ---
    """
    {"x_cols": ["E_pred_rw", "COMP_EXT"], "y_col": "E_pred_ols"},
      {"x_cols": ["E_pred_rw", "COMP_EXT"], "y_col": "E_pred_ridge"},
      {"x_cols": ["E_pred_rw", "COMP_EXT"], "y_col": "E_pred_lasso"},
      {"x_cols": ["E_pred_rw", "COMP_EXT"], "y_col": "E_pred_ann_bagging"},
      {"x_cols": ["E_pred_rw", "COMP_EXT"], "y_col": "E_pred_catboost"},
      {"x_cols": ["E_pred_rw", "COMP_EXT"], "y_col": "E_pred_gbf"},
      {"x_cols": ["E_pred_rw", "COMP_EXT"], "y_col": "E_pred_lightgbm"},
      {"x_cols": ["E_pred_rw", "COMP_EXT"], "y_col": "E_pred_rf"},
      {"x_cols": ["E_pred_rw", "COMP_EXT"], "y_col": "E_pred_xgb"},
      {"x_cols": ["E_pred_rw", "COMP_EXT"], "y_col": "COMP_LR"},
      {"x_cols": ["E_pred_rw", "COMP_EXT"], "y_col": "COMP_NL"},
      {"x_cols": ["E_pred_rw", "COMP_EXT"], "y_col": "COMP_ML"},
      {"x_cols": ["E_pred_rw", "COMP_EXT"], "y_col": "COMP_MED_LR"},
      {"x_cols": ["E_pred_rw", "COMP_EXT"], "y_col": "COMP_MED_NL"},
      {"x_cols": ["E_pred_rw", "COMP_EXT"], "y_col": "COMP_MED_ML"}

    {"x_cols": ["E_pred_rw", "E_pred_ar", "E_pred_hvz", "E_pred_ri"], "y_col": "E_pred_ols"},
      {"x_cols": ["E_pred_rw", "E_pred_ar", "E_pred_hvz", "E_pred_ri"], "y_col": "E_pred_ridge"},
      {"x_cols": ["E_pred_rw", "E_pred_ar", "E_pred_hvz", "E_pred_ri"], "y_col": "E_pred_lasso"},
      {"x_cols": ["E_pred_rw", "E_pred_ar", "E_pred_hvz", "E_pred_ri"], "y_col": "E_pred_ann_bagging"},
      {"x_cols": ["E_pred_rw", "E_pred_ar", "E_pred_hvz", "E_pred_ri"], "y_col": "E_pred_catboost"},
      {"x_cols": ["E_pred_rw", "E_pred_ar", "E_pred_hvz", "E_pred_ri"], "y_col": "E_pred_gbf"},
      {"x_cols": ["E_pred_rw", "E_pred_ar", "E_pred_hvz", "E_pred_ri"], "y_col": "E_pred_lightgbm"},
      {"x_cols": ["E_pred_rw", "E_pred_ar", "E_pred_hvz", "E_pred_ri"], "y_col": "E_pred_rf"},
      {"x_cols": ["E_pred_rw", "E_pred_ar", "E_pred_hvz", "E_pred_ri"], "y_col": "E_pred_xgb"},
      {"x_cols": ["E_pred_rw", "E_pred_ar", "E_pred_hvz", "E_pred_ri"], "y_col": "COMP_LR"},
      {"x_cols": ["E_pred_rw", "E_pred_ar", "E_pred_hvz", "E_pred_ri"], "y_col": "COMP_NL"},
      {"x_cols": ["E_pred_rw", "E_pred_ar", "E_pred_hvz", "E_pred_ri"], "y_col": "COMP_ML"},
      {"x_cols": ["E_pred_rw", "E_pred_ar", "E_pred_hvz", "E_pred_ri"], "y_col": "COMP_MED_LR"},
      {"x_cols": ["E_pred_rw", "E_pred_ar", "E_pred_hvz", "E_pred_ri"], "y_col": "COMP_MED_NL"},
      {"x_cols": ["E_pred_rw", "E_pred_ar", "E_pred_hvz", "E_pred_ri"], "y_col": "COMP_MED_ML"}
    """
    residual_configs = [
    {"x_cols": ["E_pred_rw", "COMP_MED_EXT"], "y_col": "E_pred_ols"},
      {"x_cols": ["E_pred_rw", "COMP_MED_EXT"], "y_col": "E_pred_ridge"},
      {"x_cols": ["E_pred_rw", "COMP_MED_EXT"], "y_col": "E_pred_lasso"},
      {"x_cols": ["E_pred_rw", "COMP_MED_EXT"], "y_col": "E_pred_ann_bagging"},
      {"x_cols": ["E_pred_rw", "COMP_MED_EXT"], "y_col": "E_pred_catboost"},
      {"x_cols": ["E_pred_rw", "COMP_MED_EXT"], "y_col": "E_pred_gbf"},
      {"x_cols": ["E_pred_rw", "COMP_MED_EXT"], "y_col": "E_pred_lightgbm"},
      {"x_cols": ["E_pred_rw", "COMP_MED_EXT"], "y_col": "E_pred_rf"},
      {"x_cols": ["E_pred_rw", "COMP_MED_EXT"], "y_col": "E_pred_xgb"},
      {"x_cols": ["E_pred_rw", "COMP_MED_EXT"], "y_col": "COMP_LR"},
      {"x_cols": ["E_pred_rw", "COMP_MED_EXT"], "y_col": "COMP_NL"},
      {"x_cols": ["E_pred_rw", "COMP_MED_EXT"], "y_col": "COMP_ML"},
      {"x_cols": ["E_pred_rw", "COMP_MED_EXT"], "y_col": "COMP_MED_LR"},
      {"x_cols": ["E_pred_rw", "COMP_MED_EXT"], "y_col": "COMP_MED_NL"},
      {"x_cols": ["E_pred_rw", "COMP_MED_EXT"], "y_col": "COMP_MED_ML"}
    ]
    ...
    for config in residual_configs:
        ...
        y_col = config["y_col"]
        resid_col = f"{y_col}_resid" 
        print(f"Processing configuration {count} of {len(residual_configs)}")
        count += 1
        dfs = []
        for date_val, group_df in df_merged.group_by("date"):
            dfs.append(compute_resid_info(group_df, x_cols, y_col))
        df_merged_with_resid = pl.concat(dfs)

        df_with_quintiles = create_quintile_portfolios(df_merged_with_resid, resid_col)
    ...
    results_df.write_parquet("full_results/portfolio_analysis_results.parquet") 
    print("Results saved to full_results/portfolio_analysis_results.parquet")
    return results_df
