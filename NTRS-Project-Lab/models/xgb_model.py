...
class XGBModel(BaseModel):
    """Optimized XGBoost using CPU processing"""
    ...
    def _cpu_preprocess(self, data):
        """CPU-based preprocessing using pandas"""
    ...
    def fit(self, train_data):
        train_data = self._cpu_preprocess(train_data)
    ...
    def predict(self, test_data):
        test_data = self._cpu_preprocess(test_data)
        X_test = test_data[self.input_list].values
        y_pred_scaled = self.model.predict(X_test)
        result = y_pred_scaled * test_data['csho_winsorized'].values
