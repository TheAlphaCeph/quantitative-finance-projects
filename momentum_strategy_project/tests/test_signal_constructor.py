...
class TestSignalConstructor(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.constructor = SignalConstructor(
            price_momentum_weight=0.40,
            sentiment_momentum_weight=0.35,
            frog_pan_weight=0.25,
            factor_orthogonalize=False  # Disable for testing
        )
    ...
    def test_initialization(self):
        ...
        self.assertAlmostEqual(self.constructor.price_momentum_weight, 0.40)
        self.assertAlmostEqual(self.constructor.sentiment_momentum_weight, 0.35)
        self.assertAlmostEqual(self.constructor.frog_pan_weight, 0.25)
        self.assertFalse(self.constructor.factor_orthogonalize)
    
    def test_weights_sum_to_one(self):
        """Test that signal weights sum to 1.0"""
        total_weight = (
            self.constructor.price_momentum_weight +
            self.constructor.sentiment_momentum_weight +
            self.constructor.frog_pan_weight
        )
    ...
    def test_signal_construction(self):
        ...
        signals = self.constructor.construct_signals(
            ...
            universe=self.prices.columns.tolist(),
    ...
    def test_different_weight_combinations(self):
        ...
        for w_price, w_sentiment, w_frog in weights:
            constructor = SignalConstructor(
                price_momentum_weight=w_price,
                sentiment_momentum_weight=w_sentiment,
                frog_pan_weight=w_frog,
                factor_orthogonalize=False
            )
